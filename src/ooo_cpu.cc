...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr, bool cross_pb)
{
	...
	if (L1I.PQ.occupancy < L1I.PQ.SIZE){
		...
		if(!cross_pb){
			...
			if(pf_pa == 0){
				pair<uint64_t, uint64_t> temp = va_to_pa(cpu, 0, pf_v_addr, pf_v_addr>>LOG2_PAGE_SIZE, pf_v_addr, 0, 1, 2); //changed
			...
			if(flag){
				...
				if(BPBP_FILTER){ //is 0
					...
					if(filter.first != -1){
						...
						pair<uint64_t, uint64_t> temp = va_to_pa(cpu, 0, pf_v_addr, pf_v_addr>>LOG2_PAGE_SIZE, pf_v_addr, 0, 1, 2); //changed all va_to_pa iflags to 0
						pf_pa = temp.first & (~((1 << LOG2_PAGE_SIZE) - 1)) | (pf_v_addr & ((1 << LOG2_PAGE_SIZE) - 1));
						if(pf_pa == 0){
							cout << "Error 3" << endl;
							assert(0);
					...
					pair<uint64_t, uint64_t> temp = va_to_pa(cpu, 0, pf_v_addr, pf_v_addr>>LOG2_PAGE_SIZE, pf_v_addr, 0, 1, 2); //changed here
					pf_pa = temp.first & (~((1 << LOG2_PAGE_SIZE) - 1)) | (pf_v_addr & ((1 << LOG2_PAGE_SIZE) - 1));
