...
void CACHE::handle_fill()
{
							...
							if (lower_level)
								...
								if (cache_type == IS_STLB) {
									...
									if(iflag == 0){ //changed this
										...
										answer = check_hit_stlb_pq(RQ.entry[index].address); //return pair is position in pq and pq identifier(always 0 or -1)
									}
									else{
									...
									if(answer.first == -1){
									...
									else{ //found in stlb pq
										if(PQ.entry[answer.first].free_bit == 1 && PQ.entry[answer.first].free_distance != 0){ //never here for some reason
											rfhits[1]++; 
											//cout << "HERE" << endl;
										...
										if(answer.second == 0){ //should always be true if answer.second != 0
	...
	void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
	{
		...
		/*if (packet->type == PREFETCH){
			cout << "prefetch found" << endl;
		}
		else{
			cout << packet << endl;
		}*/
		block[set][way].prefetch = (packet->type == PREFETCH) ? 1 : 0; //always 0 for some reason?
		//cout << "prefetch is " << block[set][way].prefetch << " because type is " << packet->type << endl;
		//cout << "packet type: " << dec << packet->type << endl;
		//cout << "prefetch is: " << dec << block[set][way].prefetch << hex << endl;
	...
	int CACHE::prefetch_page(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int fill_level, int pq_id, int free, int update_free, int free_distance, uint64_t id, int type, int iflag, int lad, int confidence, int irip)
	{
		...
		if(temp){ //if pa is found
			...
			pf_packet.ip = ip;
			pf_packet.type = PREFETCH; //CHANGED FROM 0xdeadbeef
	...
	int CACHE::add_pq(PACKET *packet)
	{
		...
		//cout << "here" << endl; gets here pq to cache times checks out
		PQ.TO_CACHE++;
		PQ.ACCESS++;
